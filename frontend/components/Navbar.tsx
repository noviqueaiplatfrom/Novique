...
export function Navbar({ searchQuery, setSearchQuery }: Props) {
  const { token, user, login, register, verify2fa, loginWithGoogle, logout, ready } = useAuth();
  ...
  // Google OAuth Client Loader and Button Initializer
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const google = (window as any).google;
      if (google?.accounts?.id && document.getElementById("google-signin-btn")) {
        initGoogle();
  ...
  const handleMfaSubmit = async (e: React.FormEvent) => {
    ...
    setAuthBusy(true);
    try {
      await verify2fa(mfaEmail, otpCode);
      setIsAuthModalOpen(false);
      setEmail("");
      setPassword("");
      setMfaEmail(null);
          ...
          <div className="flex items-center gap-3">
            ...
            {user && (
              <Link
                href="/profile"
                title="Profile"
                className={`p-1 rounded-xl border border-white/[0.05] transition-all hover:bg-white/[0.02] flex items-center justify-center ${
            ...
            {mfaEmail ? (
              <>
                ...
                <p className="text-xs text-textSecondary mb-6">
                  We've sent a 6-digit code to <strong className="text-zinc-200">{mfaEmail}</strong>.
                </p>

                <form onSubmit={handleMfaSubmit} className="flex flex-col gap-4">
                  <div>
                    <label className="block text-[10px] font-bold text-textSecondary uppercase tracking-wider mb-1.5">Verification Code</label>
                    <input
                      type="text"
                      required
                      maxLength={6}
                      ...
                      className="w-full h-12 text-center text-xl font-bold tracking-widest px-4 rounded-xl border border-white/[0.06] bg-white/[0.02] text-white placeholder-zinc-600 outline-none focus:border-accent/50 focus:ring-1 focus:ring-accent/20 transition-all"
                    />
                  </div>
                  ...
                  <button
                    ...
                    className="w-full h-10 rounded-xl border border-white/[0.08] text-xs font-bold text-zinc-400 hover:text-white hover:bg-white/[0.02] transition-all"
                  >
                    Back to Login
                  </button>
                </form>
              ...
              <>
                ...
                <div className="relative my-5 text-center">
                  ...
                  <span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-panel px-3 text-[10px] font-bold text-zinc-500 uppercase tracking-wider">
                    or
                  </span>
                </div>
                ...
                <div className="flex flex-col items-center gap-3">
                  {process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID ? (
                    ...
                    <button
                      type="button"
                      disabled
                      className="w-full h-10 rounded-xl border border-white/[0.08] bg-white/[0.02] text-xs font-bold text-zinc-500 flex items-center justify-center gap-2 cursor-not-allowed"
                      title="Configure NEXT_PUBLIC_GOOGLE_CLIENT_ID in your environment variables to enable"
                    >
